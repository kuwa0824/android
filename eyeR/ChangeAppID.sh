...
export APPID=io.github.kuwa0824.eyel
...
sed "s/eyer/${APPNM}/" a.txt > settings.gradle
rm a.txt

cd app
for i in AndroidManifest.xml build.gradle; do
    mv $i a.txt
    sed "s/io.github.kuwa0824.eyer/${APPID}/" a.txt > $i
done
rm a.txt
...
for i in DetectionBasedTracker_jni.cpp DetectionBasedTracker_jni.h; do
    mv $i a.txt
    sed "s/io_github_kuwa0824_eyer/${APPCC}/" a.txt > $i
done
rm a.txt
...
for i in FdActivity.java DetectionBasedTracker.java; do
    mv io/github/kuwa0824/eyer/$i ${SRCDIR}/$i
done
cd ${SRCDIR}
for i in FdActivity.java DetectionBasedTracker.java; do
    mv $i a.txt
